...
2. Follow schema-driven APIs: define or extend TypeBox schemas in `api-shared/schemas.ts`; validate all request bodies; use consistent error shapes (see api-shared/errors). Auth: request-code → user approval window → token issuance; validate token on Socket.IO connect; do not leak tokens in logs.
