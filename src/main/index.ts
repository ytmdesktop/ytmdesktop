...
import SlackStatus from "./integrations/slack";
...
const slackStatus = new SlackStatus();
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (newState.integrations.slackEnabled) {
    slackStatus.provide(store);
  }
  if (newState.integrations.slackEnabled && !oldState.integrations.slackEnabled) {
    slackStatus.enable();
    log.info("Integration enabled: Slack Status");
  } else if (!newState.integrations.slackEnabled && oldState.integrations.slackEnabled) {
    slackStatus.disable();
    log.info("Integration disabled: Slack Status");
  }
...
app.on("ready", async () => {
  ...
  // Slack
  if (store.get("integrations").slackEnabled) {
    slackStatus.provide(store);
    slackStatus.enable();
    log.info("Integration enabled: Slack Status");
  }
