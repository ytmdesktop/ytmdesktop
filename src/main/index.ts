...
import AutoSkipAd from "./integrations/auto-ad-skip";
...
const autoSkipAd = new AutoSkipAd();
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (newState.playback.autoClickSkipAd) {
    autoSkipAd.provide(ytmView);
  }

  if (newState.playback.autoClickSkipAd && !oldState.playback.autoClickSkipAd) {
    autoSkipAd.enable();
    log.info("Integration enabled: Auto skip ad");
  } else if (!newState.playback.autoClickSkipAd && oldState.playback.autoClickSkipAd) {
    autoSkipAd.disable();
    log.info("Integration disabled: Auto skip ad");
  }
...
app.on("ready", async () => {
  ...
  // AutoSkipAd
  if (store.get("playback").autoClickSkipAd) {
    autoSkipAd.provide(ytmView);
    autoSkipAd.enable();
    log.info("Integration enabled: Auto skip ad");
  }
