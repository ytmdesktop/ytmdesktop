...
import Crossfade from "./integrations/crossfade";
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (newState.playback.crossfadeEnabled) {
    crossfade.provide(ytmView);
  }
  if (newState.playback.crossfadeEnabled && !oldState.playback.crossfadeEnabled) {
    crossfade.enable();
    log.info("Integration enabled: Crossfade");
  } else if (!newState.playback.crossfadeEnabled && oldState.playback.crossfadeEnabled) {
    crossfade.disable();
    log.info("Integration disabled: Crossfade");
  }
  if (newState.playback.crossfadeDuration !== oldState.playback.crossfadeDuration) {
    crossfade.setFadeDuration(newState.playback.crossfadeDuration);
  }
...
app.on("ready", async () => {
  ...
  // Crossfade
  if (store.get("playback").crossfadeEnabled) {
    crossfade.provide(ytmView);
    crossfade.setFadeDuration(store.get("playback").crossfadeDuration);
    crossfade.enable();
    log.info("Integration enabled: Crossfade");
  }
