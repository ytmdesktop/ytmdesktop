...
app.on("ready", async () => {
  ...
  ipcMain.on("ytmView:storeStateChanged", (event, queue, likeStatus, volume, muted, adPlaying, shuffleEnabled) => {
    if (event.sender !== ytmView.webContents) return;

    playerStateStore.updateFromStore(queue, likeStatus, volume, muted, adPlaying, shuffleEnabled);
  });
