...
import { applyYtmProxyFromStore, attachProxyAuthHandler, syncYtmWebRtcForProxy, testYtmProxyFromStore } from "./proxy/apply-ytm-proxy";
import { isProxyConfigured } from "./proxy/build-proxy-config";
import { getYtmPartitionName } from "./ytm-session";
...
async function applyProxySettings(): Promise<void> {
  ...
  memoryStore.set("proxyMisconfigured", proxy.enabled && !isProxyConfigured(proxy));
  memoryStore.set("proxyApplyFailed", !(await applyYtmProxyFromStore(store)));
}
...
const createYTMView = (): void => {
  ...
  ytmView = new BrowserView({
    webPreferences: {
      sandbox: true,
      contextIsolation: true,
      partition: getYtmPartitionName(app.isPackaged),
  ...
  syncYtmWebRtcForProxy(ytmView.webContents, store.get("proxy"));
  companionServer.provide(store, memoryStore, ytmView);
  customCss.provide(store, ytmView);
  ratioVolume.provide(ytmView);
...
app.on("ready", async () => {
  ...
  let proxyYtmReloadTimer: NodeJS.Timeout | null = null;
  const reloadYtmViewForProxy = () => {
    if (!ytmView || ytmView.webContents.isDestroyed()) return;
    syncYtmWebRtcForProxy(ytmView.webContents, store.get("proxy"));
    ytmView.webContents.reload();
  };
  store.onDidChange("proxy", async () => {
    await applyProxySettings();
    if (proxyYtmReloadTimer) clearTimeout(proxyYtmReloadTimer);
    proxyYtmReloadTimer = setTimeout(() => {
      proxyYtmReloadTimer = null;
      reloadYtmViewForProxy();
    }, 300);
  });
  ...
  ipcMain.handle("settings:testYtmProxy", async event => {
    if (settingsWindow && event.sender !== settingsWindow.webContents) return { ok: false, message: "Unauthorized" };
    if (proxyYtmReloadTimer) {
      clearTimeout(proxyYtmReloadTimer);
      proxyYtmReloadTimer = null;
    }
    const result = await testYtmProxyFromStore(store);
    reloadYtmViewForProxy();
    return result;
  });
  ...
  session.fromPartition(getYtmPartitionName(app.isPackaged)).setPermissionCheckHandler((webContents, permission) => {
  ...
  session.fromPartition(getYtmPartitionName(app.isPackaged)).setPermissionRequestHandler((webContents, permission, callback) => {
    if (webContents == ytmView.webContents) {
      if (permission === "fullscreen") {
