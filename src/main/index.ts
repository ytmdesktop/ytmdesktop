...
import NowPlayingFile from "./integrations/now-playing-file";
...
const nowPlayingFile = new NowPlayingFile();
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (newState.integrations.nowPlayingFileEnabled) {
    nowPlayingFile.provide(store);
  }
  if (newState.integrations.nowPlayingFileEnabled && !oldState.integrations.nowPlayingFileEnabled) {
    nowPlayingFile.enable();
    log.info("Integration enabled: Now playing file");
  } else if (!newState.integrations.nowPlayingFileEnabled && oldState.integrations.nowPlayingFileEnabled) {
    nowPlayingFile.disable();
    log.info("Integration disabled: Now playing file");
  }
...
app.on("ready", async () => {
  ...
  ipcMain.handle("settings:showSaveDialog", async (event, options: Electron.SaveDialogOptions) => {
  ...
  // NowPlayingFile
  if (store.get("integrations").nowPlayingFileEnabled) {
    nowPlayingFile.provide(store);
    nowPlayingFile.enable();
    log.info("Integration enabled: Now playing file");
  }
