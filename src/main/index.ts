...
import SponsorBlock from "./integrations/sponsorblock";
...
store.onDidAnyChange(async (newState, oldState) => {
  ...
  if (newState.integrations.sponsorBlockEnabled) {
    sponsorBlock.provide(ytmView);
  }
  if (newState.integrations.sponsorBlockEnabled && !oldState.integrations.sponsorBlockEnabled) {
    sponsorBlock.enable();
    log.info("Integration enabled: SponsorBlock");
  } else if (!newState.integrations.sponsorBlockEnabled && oldState.integrations.sponsorBlockEnabled) {
    sponsorBlock.disable();
    log.info("Integration disabled: SponsorBlock");
  }
...
app.on("ready", async () => {
  ...
  // SponsorBlock
  if (store.get("integrations").sponsorBlockEnabled) {
    sponsorBlock.provide(ytmView);
    sponsorBlock.enable();
    log.info("Integration enabled: SponsorBlock");
  }
