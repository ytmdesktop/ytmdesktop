...
const extensionManager = new ExtensionManager(store, () => session.fromPartition(app.isPackaged ? "persist:ytmview" : "persist:ytmview-dev"));
const downloader = new Downloader(store, () => [
  mainWindow ? mainWindow.webContents : null,
  ytmView ? ytmView.webContents : null,
  settingsWindow ? settingsWindow.webContents : null
]);
...
app.on("ready", async () => {
  ...
  ipcMain.on("ytmView:loaded", event => {
    if (ytmView !== null && mainWindow !== null) {
      ...
      log.info("Received ytmView:loaded IPC, dismissing splash screen and displaying YTM view");
  ...
  ipcMain.on("ytmView:videoDataChanged", (event, videoDetails, playlistId, album, likeStatus, hasFullMetadata) => {
    ...
    // Auto download track if enabled
    downloader.handleAutoDownload(videoDetails);
  });
  ...
  // Handle extensions ipc
  ipcMain.handle("extensions:getItems", () => {
    return extensionManager.getInstalledExtensions();
  });

  ipcMain.handle("extensions:installFromUrlOrId", async (event, urlOrId: string) => {
    const ext = await extensionManager.installFromUrlOrId(urlOrId);
    if (ytmView) ytmView.webContents.reload();
    return ext;
  });

  ipcMain.handle("extensions:installFromFolder", async event => {
    const win = BrowserWindow.fromWebContents(event.sender);
    const ext = await extensionManager.installFromFolder(win);
    if (ext && ytmView) ytmView.webContents.reload();
    return ext;
  });

  ipcMain.handle("extensions:installFromArchive", async event => {
    const win = BrowserWindow.fromWebContents(event.sender);
    const ext = await extensionManager.installFromArchive(win);
    if (ext && ytmView) ytmView.webContents.reload();
    return ext;
  });

  ipcMain.handle("extensions:toggle", async (event, id: string, enabled: boolean) => {
    await extensionManager.toggleExtension(id, enabled);
    if (ytmView) ytmView.webContents.reload();
  });

  ipcMain.handle("extensions:remove", async (event, id: string) => {
    await extensionManager.removeExtension(id);
    if (ytmView) ytmView.webContents.reload();
  });

  ipcMain.handle("extensions:openFolder", async (event, id: string) => {
    await extensionManager.openFolder(id);
  });

  ipcMain.handle("extensions:reloadView", () => {
    if (ytmView) ytmView.webContents.reload();
  });

  // Handle downloader IPC
  ipcMain.on("ytmView:downloadTrack", async (event, trackInfo) => {
    try {
      const currentState = playerStateStore.getState();
      const videoDetails = currentState?.videoDetails;
      const videoId = trackInfo?.videoId || videoDetails?.id || lastVideoId;
      const title = trackInfo?.title || videoDetails?.title;
      const artist = trackInfo?.artist || videoDetails?.author;

      if (!videoId) {
        log.warn("[Downloader] Download requested but no video ID available.");
        return;
      }

      await downloader.downloadTrack({ videoId, title, artist });
    } catch (err) {
      log.error("[Downloader] Download track error:", err);
    }
  });

  ipcMain.handle("downloader:getDownloadPath", () => {
    return downloader.getDownloadDir();
  });

  ipcMain.handle("downloader:openFolder", async () => {
    await downloader.openFolder();
  });

  ipcMain.handle("downloader:selectFolder", async event => {
    const win = BrowserWindow.fromWebContents(event.sender) || settingsWindow || mainWindow;
    return await downloader.selectFolder(win);
  });
  ...
  // Setup ad-blocking network filter for YTM session
  ...
  const adUrls = [
    ...
    "*://music.youtube.com/youtubei/v1/player/ad_break*",
    "*://www.youtube.com/pagead/*",
    "*://www.youtube.com/api/stats/ads*",
    "*://www.youtube.com/youtubei/v1/player/ad_break*"
  ];

  ytmSession.webRequest.onBeforeRequest({ urls: adUrls }, (_details, callback) => {
    const isAdBlockEnabled = store.get("playback.adBlock") !== false;
    callback({ cancel: isAdBlockEnabled });
  });

  log.info("Setup ad-blocking network filter");
  ...
  // Load Chrome Extensions into YTM session
  await extensionManager.loadAll();
  log.info("Loaded Chrome extensions");
