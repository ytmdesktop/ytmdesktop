...
import listeningPartyManager from "./integrations/companion-server/api/v1/listening-party/manager";
import ListeningPartyClient from "./integrations/listening-party-client";
import os from "os";
...
const listeningPartyClient = new ListeningPartyClient();
...
app.on("ready", async () => {
  ...
  function updatePartyMemoryStore(): void {
    const info = listeningPartyManager.getPartyInfo();
    memoryStore.set("listeningPartyMembers", info ? info.members : []);
  }

  listeningPartyManager.on("memberJoined", () => updatePartyMemoryStore());
  listeningPartyManager.on("memberLeft", () => updatePartyMemoryStore());

  listeningPartyClient.setHandlers({
    onStateChanged: state => {
      memoryStore.set("listeningPartyState", state);
    },
    onMembersChanged: members => {
      memoryStore.set("listeningPartyMembers", members);
    },
    onError: message => {
      memoryStore.set("listeningPartyError", message);
      memoryStore.set("listeningPartyState", "inactive");
    },
    onDissolved: () => {
      memoryStore.set("listeningPartyState", "inactive");
      memoryStore.set("listeningPartyMembers", []);
      memoryStore.set("listeningPartyHostIp", null);
    }
  });

  ipcMain.handle("listeningParty:create", (event, displayName: string) => {
    if (settingsWindow && event.sender !== settingsWindow.webContents) return;

    if (listeningPartyClient.isConnected()) {
      return { error: "Cannot host while joined to another party" };
    }

    const { code } = listeningPartyManager.createParty(displayName || "Host");
    const localIp = getLocalIp();

    memoryStore.set("listeningPartyState", "hosting");
    memoryStore.set("listeningPartyCode", code);
    memoryStore.set("listeningPartyError", null);
    updatePartyMemoryStore();

    return { code, localIp };
  });

  ipcMain.handle("listeningParty:join", (event, hostIp: string, partyCode: string, displayName: string) => {
    if (settingsWindow && event.sender !== settingsWindow.webContents) return;

    if (listeningPartyManager.isActive()) {
      return { error: "Cannot join while hosting a party" };
    }
    if (listeningPartyClient.isConnected()) {
      return { error: "Already connected to a party" };
    }

    memoryStore.set("listeningPartyHostIp", hostIp);
    memoryStore.set("listeningPartyError", null);

    listeningPartyClient.setYtmView(ytmView);
    listeningPartyClient.connect(hostIp, 9863, partyCode, displayName || "Guest");

    return { success: true };
  });

  ipcMain.on("listeningParty:leave", event => {
    if (settingsWindow && event.sender !== settingsWindow.webContents) return;
    ...
    if (listeningPartyClient.isConnected()) {
      listeningPartyClient.disconnect();
    }

    memoryStore.set("listeningPartyState", "inactive");
    memoryStore.set("listeningPartyCode", null);
    memoryStore.set("listeningPartyMembers", []);
    memoryStore.set("listeningPartyHostIp", null);
    memoryStore.set("listeningPartyError", null);
  });

  ipcMain.handle("listeningParty:getLocalIp", event => {
    if (settingsWindow && event.sender !== settingsWindow.webContents) return;
    return getLocalIp();
  });
