...
declare const YTMD_UNLEASH_SERVER: string | null;
...
// Setup unleash server to toggle features
let unleash: Unleash = null;
let unleashSynchronized = false;
const unleashContext: Context = {
  // Defaults to random with no stickiness
...
async function setupUnleash() {
  if (YTMD_UNLEASH_SERVER) {
    ...
    try {
      ...
      try {
        unleashContext.sessionId = await fs.readFile(unleashSessionIdPath, { encoding: "utf8" });
      } catch (_) {
        /* Do nothing and just default to random every time if we couldn't properly read the file */
      }
    } catch (_) {
      await fs.writeFile(unleashSessionIdPath, unleashContext.sessionId);
    }

    unleash = new Unleash({
      url: YTMD_UNLEASH_SERVER,
      appName: app.isPackaged ? "production" : "development", // GitLab's unleash server uses appName as the environment
      instanceId: YTMD_UNLEASH_INSTANCE_ID,
      backupPath: app.getPath("userData"),
      disableAutoStart: true
    });
    unleash.on("synchronized", () => {
      log.info("Unleash: Synchronized");
      unleashSynchronized = true;
    });
    unleash.on("warn", message => {
      log.warn(`Unleash warning: ${message}`);
    });
    unleash.on("error", error => {
      log.error(`Unleash error: ${error}`);
    });
    log.info("Setup unleash");
  }
}
...
app.on("ready", async () => {
  ...
  if (unleash) {
    memoryStore.set("ytmViewLoading", true);
    memoryStore.set("ytmViewLoadingStatus", "Synchronizing feature flags...");

    await unleash.start();

    await new Promise<void>(resolve => {
      setInterval(() => {
        if (unleashSynchronized) resolve();
      }, 250);
    });
  }
